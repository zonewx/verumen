    ...
    <div className="flex-1 min-h-0 overflow-y-auto">
      ...
      <div className="max-w-4xl mx-auto w-full flex flex-col gap-6 px-6 py-6">
        ...
        <div className={`flex gap-0 border-b border-zinc-700 mb-6`}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} className={`px-5 py-2.5 text-sm font-semibold transition border-b-2 -mb-px ${tab === t.id ? 'border-zinc-300 text-zinc-100' : `border-transparent text-zinc-400 hover:text-white`}`}>{t.label}</button>
          ))}
        </div>
