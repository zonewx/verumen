...
function MarketTicker({ isDark }) {
  ...
  const labelCls = isDark ? 'text-zinc-100' : 'text-gray-700';
  const valCls   = isDark ? 'text-zinc-200' : 'text-gray-800';
...
export default function GlobalBar({ isDark, authUsername, onNavigate, onLogout, userRole, searchInputRef }) {
  ...
  return (
    <div className={`fixed top-0 right-0 z-50 h-12 flex items-center px-4 ${isDark ? 'bg-zinc-950' : 'bg-white'}`} style={{ left: 'var(--sidebar-w, 240px)' }}>
      ...
      <div ref={searchRef} className="absolute left-1/2 -translate-x-1/2 w-full max-w-md z-10">
        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-gray-50 border-gray-200'}`}>
        ...
        {results.length > 0 && (
          <div className={`absolute top-full left-0 right-0 mt-1 rounded-xl border shadow-xl overflow-hidden z-50 ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-gray-200'}`}>
            {results.map(u => {
              ...
              return (
                <button key={u.username} onClick={() => handleSelectUser(u.username)} className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition ${isDark ? 'hover:bg-zinc-800' : 'hover:bg-gray-50'} border-b ${isDark ? 'border-zinc-800' : 'border-gray-100'} last:border-0`}>
        ...
        {search.length >= 2 && !searching && results.length === 0 && (
          <div className={`absolute top-full left-0 right-0 mt-1 rounded-xl border shadow-xl px-4 py-3 text-sm ${isDark ? 'bg-zinc-900 border-zinc-800 text-zinc-400' : 'bg-white border-gray-200 text-gray-500'}`}>
      ...
      <div className="ml-auto flex items-center gap-1 shrink-0">
        ...
        <button onClick={() => onNavigate('friends')} title="Friends" className={`relative p-1.5 rounded-lg shrink-0 ${isDark ? 'text-zinc-400 hover:text-white hover:bg-zinc-900' : 'text-gray-400 hover:text-gray-900 hover:bg-gray-100'} transition`}>
        ...
        {/* Logout */}
        <button onClick={() => onLogout()} title="Sign out" className={`p-1.5 rounded-lg ml-1 shrink-0 ${isDark ? 'text-zinc-500 hover:text-red-400 hover:bg-zinc-900' : 'text-gray-400 hover:text-red-500 hover:bg-gray-100'} transition`}>
