          ...
          {cells.map((d, i) => {
            ...
            return (
              <button key={i} onClick={() => onSelect(ds)} className={`text-[11px] w-full aspect-square rounded flex items-center justify-center leading-none transition
                ${isSel ? 'bg-zinc-400 text-white font-bold' : inRange ? 'bg-zinc-500/20 text-zinc-200' : isToday ? 'border border-zinc-600 text-white' : 'text-zinc-400 hover:bg-zinc-700 hover:text-white'}`}>
                {d}
              </button>
            );
          })}
        ...
        <div className="px-4 py-3 border-b border-zinc-700/50 flex items-center gap-2">
          ...
          <div className="relative shrink-0">
            ...
            <button onClick={() => setTxFilterOpen(o => !o)}
              ...
              Filters
              {activeFilterCount > 0 && <span className="flex items-center justify-center w-4 h-4 rounded-full bg-zinc-400 text-zinc-900 text-[10px] font-bold leading-none">{activeFilterCount}</span>}
            </button>
            {txFilterOpen && (
                  ...
                  {visibleTypes.map(({ key, label }) => {
                    ...
                    return (
                      <button key={key} onClick={() => toggleType(key)}
                        ...
                        <span className={`w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0 transition ${checked ? 'bg-zinc-400 border-zinc-400' : 'border-zinc-600'}`}>
