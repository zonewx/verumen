      ...
      <div className="max-w-4xl mx-auto w-full flex flex-col gap-6">
        ...
        <div className={card}>
          <div className="flex items-center justify-between mb-4">
            <h2 className={`text-sm font-bold uppercase tracking-wider ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Market Bar</h2>
            <button
              onClick={toggleBar}
              className={`relative inline-flex h-5 w-9 shrink-0 rounded-full transition-colors duration-200 focus:outline-none ${barEnabled ? 'bg-blue-500' : isDark ? 'bg-gray-600' : 'bg-gray-300'}`}
            >
            ...
            {marketOrder.map(id => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border select-none transition ${isDraggingThis ? 'opacity-40' : ''} ${isDark ? 'border-gray-700' : 'border-gray-200'}`}
                >
