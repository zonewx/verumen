...
export default function SettingsPage({ isDark, baseCurrency, onSetBaseCurrency }) {
  ...
  return (
    <div className={`flex-1 overflow-y-auto p-6 ${isDark ? 'bg-zinc-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
      <div className="max-w-4xl mx-auto w-full flex flex-col gap-6">
        <h1 className="text-2xl font-bold">Settings</h1>

        <div className={card}>
          <h2 className={`text-sm font-bold uppercase tracking-wider mb-4 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Display</h2>
          <div>
            ...
            {marketOrder.map(id => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border select-none transition ${isDraggingThis ? 'opacity-40' : ''} ${isDark ? 'border-zinc-700' : 'border-gray-200'}`}
                >
                  <svg className={`shrink-0 cursor-grab ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`} width="14" height="14" viewBox="0 0 14 14" fill="currentColor">
                  ...
                  <span className={`text-sm font-medium ${isDark ? 'text-zinc-200' : 'text-gray-800'}`}>{idx.label}</span>
                  <span className={`ml-auto text-xs font-mono ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>{idx.ticker}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className={card}>
          <h2 className={`text-sm font-bold uppercase tracking-wider mb-4 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>CS Item Prices</h2>
          <p className={`text-sm mb-4 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>
